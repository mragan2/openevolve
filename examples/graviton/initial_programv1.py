...
def H_mg_phenomenological(a: float, m_g: float, H0: float) -> float:
    ...
    We model H_mg(a, m_g) as an effective dark-energy contribution with a
    smooth evolution across scale factor and mass. At a = 1 and m_g =
    M_G_REF, it yields Ω_mg_ref ≈ 0.7. The functional form introduces a
    mass scaling and a tapering function of the scale factor to suppress
    the term at early times and saturate at late times.

        Ω_mg(a, m_g) = Ω_mg_ref * (m_g / M_G_REF)^p * f(a),

    with p and the shape of f(a) initially set but evolvable.

    We choose f(a) = (a ** q) / (1 + (a / a_pivot) ** n), where q < 0
    suppresses early times, a_pivot is the scale factor at the inflection
    point, and n controls the sharpness of the transition. All parameters
    (p, q, n, a_pivot) can be adjusted by the evolutionary process.
    """
    if m_g <= 0.0:
        return 0.0
    ...
    # Evolvable parameters controlling mass dependence and scale-factor shape
    p = 2.0
    q = -1.0
    n = 2.0
    a_pivot = 0.5

    # Smooth tapering function: small at early a, saturates as a approaches 1
    f_a = (a ** q) / (1.0 + (a / a_pivot) ** n)

    Omega_mg = Omega_mg_ref * (x ** p) * f_a
...
def lambda_eff_from_mg(m_g: float) -> float:
    ...
    We use a saturating power-law scaling to ensure correct dimensions and
    allow smooth evolution in m_g. At m_g = M_G_REF, the value is
    λ_ref ≈ 1e-52 m^-2.

        Λ_eff(m_g) = λ_ref * (x ** alpha) / (1 + x ** beta),

    where x = m_g / M_G_REF. The exponents alpha and beta are initially
    set but can evolve to tune the mass dependence. This form keeps
    Λ_eff finite for small m_g and reduces to λ_ref at the reference mass.
    """
    if m_g <= 0.0:
        return 0.0
    ...
    return lambda_ref * (x ** alpha) / (1.0 + x ** beta)
# EVOLVE-BLOCK-END
